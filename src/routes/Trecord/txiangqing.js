...
import { ALL_ITEMS } from 'shufflejs';
...
class ButtonDemo extends React.Component {
  constructor(props) {
    ...
    //  let obj=JSON.parse(local1Storage.getItem(ksid));
    this.state = {
      panduanArr: [],
      lilundanArr:[],
      lilunxuanArr:[],
  ...
  componentDidMount() {
    ...
    let userdata = this.props.location.state||localStorage.getItem('tksid');
    $.ajax({
      url: `${url}` + '/index/addpaper/xiangqing',
      type: 'get',
      dataType: 'JSON',
      data: { id: userdata.tksid },
      success: (data) => {
        console.log(data)
  ...
  getPanduanListHtml () {
    ...
    <Form.Item  layout="inline">
          ...
          <div>
          {item.daan == '1'?<div><span>答案：</span><span style={{color:"yellowgreen"}}>正确</span></div>:<div><span>你的答案：</span><span style={{color:"red"}}>错误</span></div>}
          </div>
          <div>
